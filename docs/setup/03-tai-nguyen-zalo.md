...
4. Token này đặt vào `ZALO_BOT_TOKEN` trong `/opt/zino/.env` trên VPS (OpenClaw đọc env này cho account default).
...
- Group: `groupPolicy: open` — ai trong nhóm cũng gọi được Zino qua @mention.
...
Tạo tại console.anthropic.com → đặt vào `ANTHROPIC_API_KEY` trong `/opt/zino/.env` (model cho agent Zino).
